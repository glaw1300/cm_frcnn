...
"""
def get_plume_info(plumes, tif, lat_name="plume_lat", lon_name="plume_lon", pid_name="plume_id",
                    q_name="qplume", qs_name="sigma_qplume"):
    # get all unique plumes
    pxs = []
    pids = []
    psizes = []
    src = None
    ...
    for ind, row in plumes.iterrows():
        ...
        pxs.append((x,y))
        pids.append(row[pid_name])
        psizes.append((row[q_name], row[qs_name]))

    # return lat long of all unique plumes
    return pxs, pids, psizes
...
"""
def get_prediction_outputs(tifs, cnn, score_thresh, nms_thresh = .25):
...
def determine_source(bboxes, scores, classes, loc, labels):
    ...
    if best_src[0] == "processing":
        try:
            next_best = torch.topk(dists/scores, 2, largest=False).indices[-1]
            best_src = labels[int(classes[next_best].item())], dists[next_best].item(), scores[next_best].item()
            #print(f"Detected processing, now taking {best_src[0]} @ {best_src[1]},{best_src[2]}")
        except:
            pass
            #print(f"Only processing to take: {bboxes}")
    ...
    for n in range(iters):
        ...
        for name, group in tqdm.tqdm(df.groupby(["source_id"])):
            ...
            if plot and n == 0:
                ...
                fig, ax = plotter.plot_pred_boxes(cv2.imread(tifs[0]), {"instances":tmp}, cnn.get_labels(), show=False)
            ...
            for i, ((x,y), pid, psize) in enumerate(zip(cents, pids, psizes)):
                ...
                if plot and n == 0:
                    ...
                    ax.add_patch(Circle((x,y), psize[0]/200., fc=c, ec="black"))
            ...
            if plot and n == 0:
                fig.savefig(f"preds_w_src/{sid}.png", dpi=100)
                plt.close(fig)
                """
                if gt_src == pred_src:
                    success += 1
        ...
        with open(f"compares/compare_{n}.pickle", "wb") as f:
            pickle.dump(preds, f)
